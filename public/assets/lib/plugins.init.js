...
try {
    var saveProfileImage = function (event, type) {
        var image = document.getElementById(event.target.name);
        image.src = URL.createObjectURL(event.target.files[0]);
        const file = event.target.files[0];
        const provider = event.target.dataset.provider; // Get the user data from the data attribute
       
        
        
        const formData = new FormData();
        formData.append(type + '-image', file);
        formData.append('provider', provider); // Append the user data

        axios.post('/save-profile-image', formData, {
            headers: {
...
tinymce.init({
    ...
    'lists media',
    'paste' ],
toolbar: 'undo redo | styleselect | bold italic | alignleft aligncenter alignright alignjustify',    tinycomments_mode: 'embedded',
